...
(async function (){
    ...
    const bananita = await Product.findOne({title: "Banananita"}) as Product

    const giovanna = await Product.findOne({title: "giovanna"}) as Product

    const order = new Order("25563", [bananita, giovanna])
    await order.save()
